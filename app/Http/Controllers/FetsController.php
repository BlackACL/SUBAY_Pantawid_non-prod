...
public function submittedEmbed()
{
    $documents = FetsDocument::where('user_id', auth()->id())
        ->with(['submitter', 'verifier', 'approver', 'fetsItems'])
    ...
    public function generate(Request $request)
    {
        ...
        try {
            ...
            // 5b. Create fets_items records for each selected property
            ...
            // 5c. Log the submission
            FetsLog::create([
                'property_no' => implode(',', $validated['selected']),
                'action'      => 'submitted',
                'actor'       => $user->fullname,
                'actor_role'  => $user->access_level,
                'remarks'     => "{$movement} FETS #{$fets->id} submitted by {$user->fullname}",
            ]);
...
public function update(Request $request)
{
    ...
    try {
        ...
        $itemCount = count($allItems);
        $userOffice = $this->getAbbreviatedOffice($user->office ?? 'Pantawid (RPMO)');
        $finalRemarks = ($movement === 'For Repair') ? 'Repair' : $remarks;
        ...
        // Update FETS with new file info
        $fets->update([
            'file_name' => $fileName,
            'file_path' => $filePath,
        ]);
...
private function getHeadOfProperty(): ?\App\Models\Official
{
    ...
    public function reviewSubmitted()
    {
        $documents = FetsDocument::where('status', 'submitted')
            ->with(['submitter', 'fetsItems']) // ✅ Load the submitter and fetsItems relationships
            ->orderByDesc('created_at')
            ->paginate(10);
...
{
    $query = FetsDocument::where('status', 'verified')
        ->with(['submitter', 'verifier', 'approver', 'fetsItems']); // ✅ Load the submitter and fetsItems relationships
