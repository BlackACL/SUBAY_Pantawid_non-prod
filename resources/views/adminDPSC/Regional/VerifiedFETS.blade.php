                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="w-[14%] px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex justify-center items-center space-x-2">
                                        <!-- Preview Button - Updated Design -->
                                        <button @click="pdfUrl = '{{ route('fets.preview', $doc->id) }}?t=' + Date.now(); showModal = true"
                                        ...
                                        <!-- Approve Button -->
                                        <form method="POST" action="{{ route('fets.approve', $doc->id) }}" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                onclick="return confirm('Are you sure you want to approve this FETS? This will transfer the equipment.')"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center">
                                    ...
                                    <h3 class="mt-2 text-sm font-medium text-gray-900">No verified FETS requests</h3>
                                    <p class="mt-1 text-sm text-gray-500">All verified FETS will appear here.</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
...
<script>
    ...
    // Function to open PDF in new tab when iframe fails
    function openPdfInNewTab() {
        const modal = document.querySelector('[x-show="showModal"]');
        if (modal) {
            const pdfUrl = modal.__x.$data.pdfUrl;
            if (pdfUrl) {
