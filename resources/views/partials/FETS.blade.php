    ...
    <div class="bg-white p-6 shadow-sm rounded-lg">
        ...
        <form action="{{ $formAction }}" method="POST" id="fetsForm">
            ...
            <div class="mb-4">
                ...
                <select id="transfer_movement" name="transfer_movement" required
                    ...
                    @if(auth()->user()->access_level === 'Provincial DPSC' || auth()->user()->access_level === 'Regional DPSC')
                        {{-- Provincial/Regional users: Issue/Transfer instead of Return to Lender --}}
                    ...
                    @else
                        ...
                        <option value="Return to Lender" {{ old('transfer_movement', $prefilledData['transfer_movement'] ?? '') == 'Return to Lender' ? 'selected' : '' }}>Return to Lender</option>
                    @endif
            ...
            <div id="remarks_wrapper"
                ...
                @if($isEditMode && isset($editFets->remarks))
                    <p class="text-sm text-gray-600 mt-2">
                        <span class="font-semibold">Original Remarks:</span> 
                        <span class="text-blue-600">{{ $editFets->remarks }}</span>
                    </p>
                @endif
            </div>
            ...
            <div class="mb-4">
                <label class="block font-medium text-sm text-gray-700">
                    Select Equipment (max 5)
                    <span id="selectedCount" class="ml-2 text-blue-600 font-semibold"></span>
                </label>
...
<tbody>
    @forelse ($inventory as $item)
        @php
            // Check if this item is currently being edited in THIS FETS
            $isBeingEditedInThisFets = in_array($item->PROPERTY_NO, $editingFetsPropertyNos ?? []);
            
            // Only disable if locked in ANOTHER FETS (not the one being edited)
            $disabledGeneral = in_array($item->PROPERTY_NO, $inProcessPropertyNos ?? []) && !$isBeingEditedInThisFets;
            $disabledRepair  = in_array($item->PROPERTY_NO, $repairInProcessPropertyNos ?? [])
                               && !in_array($item->PROPERTY_NO, $returnedFromRepairPropNos ?? [])
                               && !$isBeingEditedInThisFets;
        ...
        <tr class="{{ $disabledGeneral || $disabledRepair ? 'bg-gray-100 text-gray-500 italic' : '' }}">
            <td class="p-2 text-center">
                ...
                @else
                    {{-- Item can be selected/deselected --}}
                    ...
                    @if ($isBeingEditedInThisFets)
                        <span class="block text-xs mt-1 text-blue-600 font-semibold">
                            (Currently in this FETS)
                        </span>
                    @endif
                @endif
            </td>
...
<script>
    ...
    function updateFormFields(choice) {
        ...
        } else if (choice === 'Return to Lender') {
            remarksWrapper.classList.remove('hidden');
            // Receiver stays hidden for Return to Lender
            remarksSelect.required = true;
            remarksSelect.value = 'Serviceable';

        } else if (choice === 'Issue/Transfer') {
            remarksWrapper.classList.remove('hidden');
            receiverWrapper.classList.remove('hidden'); // Show receiver for Issue/Transfer
            remarksSelect.required = true;
            remarksSelect.value = 'Serviceable';

        } else if (choice === 'For Surrender') {
            remarksWrapper.classList.remove('hidden');
            // The receiverWrapper is NOT shown, it remains hidden as per the default state.
            remarksSelect.required = true;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateSelectedCount() {
            const checkboxes = document.querySelectorAll('input.select-checkbox:checked');
            ...
            if (countElement) {
                if (count > 0) {
                    ...
                    countElement.classList.add(count > 5 ? 'text-red-600' : 'text-green-600');
                } else {
        ...
        document.querySelectorAll('input.select-checkbox').forEach(checkbox => {
